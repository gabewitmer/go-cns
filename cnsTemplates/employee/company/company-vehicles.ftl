			...
			<div class="row">
				...
				<div class="col-lg-12">
					<ol class="breadcrumb">
						...
						<li>
							<a href="/employee/company">Company</a>
						</li>
						<li>
							<a href="/employee/company/${(company.id)!}">${(company.name)!}</a>
						</li>
						<li class="active">
							${(vehicle??)?string('<a href="/employee/company/${(company.id)!}/vehicle">Vehicles</a>','Vehicles')!}
						</li>
						<#if !addnew?? && !vehicle??>
							<li>
								<a href="/employee/company/${(company.id)!}/vehicle?addnew"><i class="fa fa-plus">&nbsp;Add New</i></a>
							</li>
						<#else>
				...
				<div class="col-lg-4">
					<div class="panel panel-default">
						<div class="panel-heading clearfix">
							...
							<a href="/employee/company/${(company.id)!}/vehicle" class="btn btn-default btn-xs pull-right">Cancel</a>
						</div>
						<div class="panel-body">
							<form class="form-horizontal" action="/employee/company/${(company.id)!}/vehicle" method="post">
								...
								<div class="col-lg-12">
									...
									<#if vehicle??>
										<div class="form-group">
											<a href="#" data-message="Are you sure you would like to permanently delete this vehicle?" data-delete="/employee/company/${(company.id)!}/vehicle/${(vehicle.id)!}" class="btn btn-danger btn-block delete-button">Delete</a>
										</div>
									</#if>
								</div>
							</form>
								...
								<table id="vehicles" class="table table-bordered table-striped">
									...
									<tbody>
										<#list vehicles as vehicle>
											<tr>
												<td>
													<a href="/employee/company/${(company.id)!}/vehicle/${(vehicle.id)!}" class="btn btn-primary btn-xs">${(vehicle.id)!}</a>
												</td>
												<td>${(vehicle.name)!}</td>
